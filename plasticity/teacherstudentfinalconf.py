...
def getepochsforstudent(epoch, teacher_epochs, total_student_epochs, minepoch):
    return int(
        minepoch
        + (total_student_epochs - minepoch)
        * 0.5
        * (1 - math.cos(math.pi * epoch / (teacher_epochs - 1)))
    )
...
if __name__ == "__main__":
    ...
    random_noise = jax.random.uniform(
        key,
        shape=(noise_amount_step * eras, 784),
        minval=-math.sqrt(3),
        maxval=math.sqrt(3),
    )

    key2 = jax.random.PRNGKey(69)
    random_noise_test = jax.random.uniform(
        key2, shape=(40000, 784), minval=-math.sqrt(3), maxval=math.sqrt(3)
    )
    ...
    for era in range(eras):
        ...
        teacher_loss = model_teacher.loss(train_teacher_x, train_teacher_y)
        loss_teacher.append(teacher_loss)
        ...
        for i in range(era + 1):
            ...
            random_noise_step = random_noise[
                (i % 30) * noise_amount_step : ((i % 30) + 1) * noise_amount_step
            ]
            print(random_noise_step.device)
            train_student_y = model_teacher.forward(
                model_teacher.params, random_noise_step
            )
        ...
        student_loss = model_student_along.loss(train_teacher_x, train_teacher_y)
        loss.append(student_loss)
        ...
        # print(deads)
        along_student_data = brightschtudent.forward(
            brightschtudent.params, random_noise_test
        )
