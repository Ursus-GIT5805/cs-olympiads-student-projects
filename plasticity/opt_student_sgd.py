...
if __name__ == '__main__':
    ...
    # THIS IS CRAP ADAMW WORKS SO MUCH BETTER!
    # [sgd lr: 0.5; mom: 0.5; bs: 125]: 0.06590502709150314

    learning_rates = [0.4375, 0.45, 0.4625]
    momentums = [0.4375, 0.45, 0.4625]
    batch_sizes = [75]
    ...
    for lr in learning_rates:
        for mom in momentums:
            for bs in batch_sizes:
                to_try.append({"lr": lr, "mom": mom, "bs": bs})
    ...
    for trying in to_try:
        ...
        optimizer_student_live = optax.sgd(learning_rate=lr, momentum=mom)
        ...
        for era in range(teacher_eras):
            print("Era [sgd lr: {}; mom: {}; bs: {}]: {}/{}".format(lr, mom, bs, era+1, teacher_eras))
        ...
        test_noise_y_student = model_student_live.evaluate(test_noise)
        kl_div = kl_divergence(p=test_noise_y_teach, q=test_noise_y_student)
        print("[sgd lr: {}; mom: {}; bs: {}]: {}".format(lr, mom, bs, kl_div))
        results.append((lr, mom, bs, kl_div))
    
    results_sorted = sorted(results, key = lambda x: x[3], reverse=True)
    for lr, mom, bs, kl_div in results_sorted:
        print("[sgd lr: {}; mom: {}; bs: {}]: {}".format(lr, mom, bs, kl_div))